...
Page({
  ...
  getAll: function () {
    ...
    query.equalTo("owner", wx.getStorageSync("user_id"));
    query.count({
   ...
   fetchPostsData: function (data, endpage) {
    ...
    query.equalTo("owner",wx.getStorageSync("user_id")); //查询出联系表中是我的记录wx.getStorageSync("user_id")
    query.include("order");
    query.include("space");
    ...
    query.find({
      success: function (results) {
        for (var i = 0; i < results.length; i++) {
          ...
          var username = results[i].get("userrealname");
